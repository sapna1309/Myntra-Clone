          ...
          >
           ...
           <Stack>
            <Box  border={"1px solid lightgray"} p={2} >
                <HStack>
                  <Image src="https://w7.pngwing.com/pngs/679/616/png-transparent-sales-discounts-and-allowances-computer-icons-dicount-miscellaneous-angle-text.png" w="30px"  />
                  <Text fontWeight={700} fontSize="14px" >Bank Offer</Text>
                </HStack>
                <Text margin={"10px 0px"} textAlign={"left"} paddingLeft="40px" color="gray" fontSize={"13px"} >• 10% Instant Discount on Credit and Debit Card on a min Spend of Rs 3,000. TCA </Text>
                <Text textAlign={"left"} paddingLeft="40px" fontWeight={700} fontSize="14px" color={"#ff3f6c"}>Show More</Text>
            </Box>

            <Box w="full" >
              <Heading textAlign={"left"} fontSize="16px" p={"10px 0"} >Choose Payment Mode</Heading>
              ...
              toggle ?
                ...
                <Box textAlign={"left"} pl={4} >
                  ...
                  <Text m={"10px 0px"} fontSize="14px" fontWeight={"700"}  >CREDIT/DEBIT CARD</Text>
                  <Text fontSize={"14px"} color="darkgray" >please ensure your card can be used or online transaction .<Text color="#ff3f6c" >know More</Text></Text>
                  <Stack mt={4} spacing={6} >
                    
                    <Input fontSize={"13px"} type="number"  placeholder='Card Number' isRequired 
                     onChange={(e)=>setValue((prev)=>({...prev, cardno:e.target.value }))}
                    />
                    <Input fontSize={"13px"} type="text" marginTop={4} marginBottom={4}  placeholder='Name on card' isRequired 
                    onChange={(e)=>setValue((prev)=>({...prev, cardName:e.target.value }))}
                      />
                    <HStack >
                    <Input fontSize={"13px"} type="month"  placeholder='Valid Thru(MM/YY)' isRequired
                    onChange={(e)=>setValue((prev)=>({...prev, month:e.target.value }))}
                     />
                    <Input fontSize={"13px"} type="number"  placeholder='CVV' isRequired 
                     onChange={(e)=>setValue((prev)=>({...prev, cvv:e.target.value }))}
                    />
                    </HStack>
                    <Button mt={4} background={"#ff3f6c"} color="#fff"
                    w="full" 
                     _hover={{
                      backgrounColor:"#fff36c"
                     }}
